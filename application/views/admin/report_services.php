<?php defined('BASEPATH') OR exit('No direct script access allowed');
  require('report_head.php');
  if ($xdate != '' OR $ydate != '') {
    $cats = [];
    $temp = $this->rgm_model->find_alls('services');
    foreach ($temp as $row) $cats[$row['id']] = $row['name'];

    $this->db->select('service, SUM(`price` * `quantity`) AS amount');
    if ($xdate) $this->db->where('date >=', $xdate);
    if ($ydate) $this->db->where('date <=', $ydate);
    $this->db->group_by('service');
    $sums = $this->rgm_model->find_all('tasks');
    $dater = 'Services Report for: &nbsp; '.$dater;
?>
<div class="banner pb-3 align-items-end d-print-none">
  <?php require('report_date.php'); ?>
  <div class="dropdown">
    <button class="btn dropdown-toggle btn-outline-yfarms py-2" data-bs-toggle="dropdown">Download</button>
    <div class="dropdown-menu dropdown-menu-end">
      <a href="javascript:void(0)" class="dropdown-item" onclick="window.print()">Print PDF</a>
      <a href="javascript:void(0)" onclick="toExcel('excel', '<?= $dater ?>')" class="dropdown-item">Excel</a>
    </div>
  </div>
</div>
<div class="container border pb-3 mb-3">
  <div class="row g-0">
    <div class="col-12"><h5 class="mb-0 py-2">Summary</h5></div>
  <?php foreach ($sums as $row): ?>
    <div class="col-md-6 banner p-2 border"><?= $cats[$row['service']]; ?> <b class="money"><?= number_format($row['amount']); ?></b></div>
  <?php endforeach ?>
  </div>
</div>
<div class="container px-0">
  <table class="table table-bordered" id="excel">
    <thead>
      <tr class="d-none d-print-table-row text-center">
        <th colspan="8" style="font-family: monospace;"><?= $dater; ?></th>
      </tr>
      <tr>
        <th>Date</th>
        <th class="text-nowrap">Service ID</th>
        <th>Code</th>
        <th>Service</th>
        <th class="text-center">Price</th>
        <th width="40px">Quantity</th>
        <th class="text-center">Subtotal</th>
        <th class="text-center">Balance</th>
      </tr>
    </thead>
    <tbody>
  <?php
    $this->db->select('voucher, COUNT(*) AS count')->group_by('voucher');
    $data = $this->rgm_model->find_all('tasks'); $vouchers = [];
    foreach ($data as $key) $vouchers[$key['voucher']] = $key['count'];
    $this->db->select('date, COUNT(*) AS count')->group_by('date');
    $data = $this->rgm_model->find_all('tasks'); $dates = [];
    foreach ($data as $key) $dates[$key['date']] = $key['count'];

    $this->db->select('service, task_id, voucher, name, phone, price, quantity, date');
    $this->db->order_by('date, voucher');
    if ($xdate) $this->db->where('date >=', $xdate);
    if ($ydate) $this->db->where('date <=', $ydate);
    $data = $this->rgm_model->find_all('tasks');
    $total = 0; 
    $voucher = $date = '';
    foreach ($data as $row) {
      $subtotal = $row['price'] * $row['quantity'];
      $total += $subtotal;
  ?>
      <tr>
      <?php if ($date != $row['date']){ ?>
        <td class="text-nowrap text-center" rowspan="<?= $dates[$row['date']]; ?>"><?= date('d M, Y', strtotime($row['date'])); ?></td>      
      <?php $date = $row['date']; } ?>
      <?php if ($voucher != $row['voucher']){ ?>
        <td rowspan="<?= $vouchers[$row['voucher']]; ?>" class="text-center"><?= $row['voucher']; ?></td>
      <?php $voucher = $row['voucher']; } ?>
        <td><?= $row['task_id']; ?></td>
        <td><?= $cats[$row['service']]; ?></td>
        <td class="text-end"><span class="money"><?= number_format($row['price']); ?></span></td>
        <td class="text-end"><?= $row['quantity']; ?></td>
        <td class="text-end"><span class="money"><?= number_format($row['price'] * $row['quantity']); ?></span></td>
        <td class="text-end"><span class="money"><?= number_format($total); ?></span></td>
      </tr>
  <?php } ?>
  <?php if (count($data) < 1) { ?>
      <tr><td colspan="8" class="p-5 text-danger text-center">No data available</td></tr>
  <?php } else { ?>
      <tr><td colspan="8" class="p-3 text-center"><h5 class="mb-0">Total: &nbsp; <b class="money"><?= number_format($total); ?></b></h5></td></tr>
  <?php } ?>
    </tbody>
  </table>
</div>
<?php } ?>
